...
import 'package:moovup_program_test/Common/Widgets/PersonMarker.dart';
...
void main() {
  ...
  testWidgets('PeopleListView displays people information',
    ...
    provideMockedNetworkImages(() async {
      ...
      await gesture.moveBy(const Offset(0, -360)); //How much to scroll by
      await tester.pump();
      expect(find.text('Susanna Patel'), findsOneWidget);
    });
  });

  testWidgets('PeopleListView got error', (WidgetTester tester) async {
    provideMockedNetworkImages(() async {
      //Setup
      List<dynamic> decodedJson = jsonDecode(json);
      List<PersonModel> people = decodedJson
      ...
      final PeopleListViewModel peopleListViewModel = PeopleListViewModel();
      peopleListViewModel.people = people;
      peopleListViewModel.errorMessage = "Not found";
      peopleListViewModel.errorCode = 404;
      ...
      expect(find.text('Amparo Calhoun'), findsNothing);
      expect(find.text('Not found'), findsOne);
    });
  });

  testWidgets('PeopleListView pull down to refresh',
      (WidgetTester tester) async {
    provideMockedNetworkImages(() async {
      ...
      //Check result
      await tester.fling(
          find.text('Amparo Calhoun'), const Offset(0.0, 120.0), 500.0);
  ...
  testWidgets('MapView displays people information',
      (WidgetTester tester) async {
    provideMockedNetworkImages(() async {

      //Setup
      final MapViewModel mapViewModel = MapViewModel();
      final PeopleListModel peopleListModel = PeopleListModel();
      ...
      peopleListModel.setPeople([personModel]);

      //Run
      await tester.pumpWidget(
        MaterialApp(
          home: ChangeNotifierProvider<PeopleListModel>(
            create: (_) => peopleListModel,
            child: MapView(mapViewModel: mapViewModel),
          ),
        ),
      );
      ...
      //Check result
      expect(mapViewModel.people.length, 1);
      expect(find.byType(FlutterMap), findsOne);
      expect(find.byType(Icon), findsOne);

      //Update PeopleListModel
      peopleListModel.setPeople([personModel,personModel2]);
      await tester.pumpAndSettle();

      //Check result
      expect(mapViewModel.people.length, 2);

    });
  });

  testWidgets('DetailView displays people information',
      (WidgetTester tester) async {
    provideMockedNetworkImages(() async {
      //Setup
      NameModel nameModel = NameModel(first: "Chan", last: "Tai Man");
      ...
      final DetailViewModel detailViewModel =
          DetailViewModel(person: personModel);
      ...
      PersonModel personModel2 = PersonModel(
          ...
          email: "124@2345.com",
          location: locationModel2);
      peopleListModel.setPeople([personModel2]);
      ...
      await tester.pumpWidget(MaterialApp(
        ...
        home: ChangeNotifierProvider<PeopleListModel>(
          create: (_) => peopleListModel,
          child: DetailView(detailViewModel: detailViewModel),
        ),
      ));

      //Check result
      expect(find.text('Chan Tai Man'), findsOneWidget);
      expect(find.text('124@234.com'), findsOneWidget);
      expect(find.byType(FlutterMap), findsOneWidget);

      //Check DetailView will not affect by update PeopleListModel
      peopleListModel.setPeople([personModel2]);
